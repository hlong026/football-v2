...
export function splitCompanies(value) {
    return value
        .split(/[\r\n,，;；、]+/)
        .map((item) => item.trim())
        .filter(Boolean);
}
export function splitCookies(value) {
    return value
        .split(/\r?\n/)
        .map((item) => item.trim())
        .filter(Boolean);
...
export function isNonCriticalMatchFailure(failedPages, parseSummary, fetchedPages, expectedEuropeanCount = expectedEuropeanTargetCount, expectedAsianCount = expectedAsianTargetCount) {
...
export function trimLeadMarker(line) {
...
export function cutText(value, maxLength) {
...
function compactDecisionLine(line) {
...
function isMethodologyLine(line) {
...
function pickBestDecisionLine(lines) {
...
function extractFinalConclusion(rawLines, cleanedLines) {
...
function extractConclusionLead(rawLines, paragraphs, cleanedLines, headline) {
    for (let index = 0; index < rawLines.length; index += 1) {
        const currentLine = formatAnalysisText(rawLines[index]).trim();
...
export function buildAnalysisSectionCards(view) {
    const configs = [
        {
            key: 'tendency',
            title: '倾向方向',
    ...
    const buckets = Object.fromEntries(configs.map((config) => [config.key, []]));
    ...
    const fallbackOrder = ['tendency', 'basis', 'risk', 'advice'];
...
export function barStyle(value) {
...
export function prettyDisplayPreferred(value) {
